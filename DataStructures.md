...
```
### **线段树
<img width="671" height="462" alt="image" src="https://github.com/user-attachments/assets/d218b23d-60eb-4e0e-ad25-0357ebc9198a" />
...
```c++
// 线段树类，用于维护区间最大值，并支持查找并更新第一个 >= x 的元素
class SegmentTree {
    vector<int> mx;  // 存储线段树每个节点对应区间的最大值

    // 维护当前节点 o 的值：从其左右子节点中取最大值更新自己
    void maintain(int o) { // 该函数用于在子节点更新后，向上回溯更新父节点
        mx[o] = max(mx[o * 2], mx[o * 2 + 1]);  // 左子树：o*2，右子树：o*2+1
    }

    // 递归构建线段树
    // 参数：原数组 a，当前节点编号 o，当前节点对应的区间 [l, r]
    void build(const vector<int>& a, int o, int l, int r) {
        if (l == r) {// 叶子节点：区间只有一个元素，直接赋值
            mx[o] = a[l];
            return;
        }
        int m = l + (r - l) / 2;  // 中点，划分区间
...
public:
    // 构造函数：根据输入数组 a 初始化线段树
    SegmentTree(const vector<int>& a) {
        size_t n = a.size();
        // 预留足够的空间存储线段树节点
        // 2 << bit_width(n-1) 等价于找到大于等于 n 的最小2的幂，再乘以2
        // 这是为了保证线段树数组足够大，避免越界
        // bit_width(n-1) 返回 n-1 的二进制位数，2 << k 相当于 2^(k+1)
        mx.resize(2 << bit_width(n - 1));

        // 从根节点 o=1 开始，构建覆盖区间 [0, n-1] 的线段树
        build(a, 1, 0, n - 1);
    }

    // 查询并更新：在整棵树中查找第一个 >= x 的元素，将其值更新为 -1，并返回其原始下标
    // 参数：当前节点 o，当前节点对应区间 [l, r]，目标值 x
    // 返回值：找到的第一个 >= x 的元素的下标；若不存在，返回 -1
    int findFirstAndUpdate(int o, int l, int r, int x) {
        if (mx[o] < x) {
            // 当前区间最大值都小于 x，说明没有 >= x 的元素，直接返回 -1
            return -1;
        }
        if (l == r) {
            // 到达叶子节点，且 mx[o] >= x（因为上面没返回）
            // 找到目标位置，将其值更新为 -1（表示该位置不再可用）
            mx[o] = -1;
            return l;  // 返回该元素的下标
        }

        int m = l + (r - l) / 2;   // 计算中点，划分区间
        ...
        // 无论从哪边找到，回溯时都要维护当前节点的最大值
        maintain(o);

        return i;  // 返回找到的下标（-1 表示未找到）
    }
};
